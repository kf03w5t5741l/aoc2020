...
struct Seatmap* next_seatmap(struct Seatmap* seatmap)
{
    ...
    for (int y = 0; y < seatmap->rows; y++) {
        for (int x = 0; x < seatmap->columns; x++) {
            int start_row = y - 1 >= 0 ? y - 1 : 0;
            int end_row = y + 1 < seatmap->rows ? y + 1 : seatmap->rows - 1;

            int start_column = x - 1 >= 0 ? x - 1 : 0;
            int end_column = x + 1 < seatmap->columns? x + 1 : seatmap->columns - 1;

            int occupied = 0;

            for (int row = start_row; row <= end_row; row++) {
                for (int column = start_column; column <= end_column; column++) {
                    if (seatmap->seats[row][column] == '#') {
                        occupied++;
                    }
                }
            }
