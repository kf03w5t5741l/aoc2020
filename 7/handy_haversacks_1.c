...
void check_rules_recurse(struct Bag* bag, struct Bag* search, int* result)
{
    if (bag == NULL) {
        return;
    }
    ...
    while (rule != NULL) {
        ...
        if (ht_equals(rule->bag, search)) {
            (*result)++;
            return;
        }
        check_rules_recurse(rule->bag, search, result);
        rule = rule->next;
    }
}
...
int main()
{
    ...
    for (int i = 0; i < TABLE_SIZE; i++) {
        ...
        int can_hold = 0;
        check_rules_recurse(bagtable[i]->bag, &search, &can_hold);

        if (can_hold) {
            viable_bags++;
        }
    }
    printf("Viable bags: %d\n", viable_bags);
